...
#define MAX_SLOT_SIZE (1024 * 1024)

static control_t test_malloc(const size_t call_count)
{
    int total = 0;
    void* pointer = NULL;
    int chunk = MAX_SLOT_SIZE;

    /**
     * Try allocating a large chunck, decrease chunk size until one fits.
     */
    while (chunk > 0) {

        pointer = malloc(chunk);
...
Case cases[] = {
    Case("malloc - few large allocations", test_malloc),
};
